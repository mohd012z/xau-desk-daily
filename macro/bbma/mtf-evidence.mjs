...
export const BBMA_TIMEFRAMES=Object.freeze(['D1','H4','H1','M30','M15','M5']);
const ROLES=Object.freeze({D1:'MAJOR_CONTEXT',H4:'PRIMARY_STRUCTURE',H1:'CONFIRMATION',M30:'REFINEMENT',M15:'PRIMARY_SETUP',M5:'EARLY_CONFIRMATION'});

function missing(tf,ruleVersion) {
  return Object.freeze({timeframe:tf,role:ROLES[tf],rule_version:ruleVersion,direction:'NEUTRAL',status:'INSUFFICIENT_DATA',buy_detectors:Object.freeze([]),sell_detectors:Object.freeze([]),observations:Object.freeze([])});
}

export function aggregateBbmaEvidence({ seriesByTimeframe={}, ruleVersion='bbma-shadow-v1' }={}) {
  if (!seriesByTimeframe || typeof seriesByTimeframe!=='object' || Array.isArray(seriesByTimeframe)) throw new TypeError('seriesByTimeframe must be an object');
  ...
  return Object.freeze({rule_version:ruleVersion,overall_state,timeframes,buy_timeframes:buy,sell_timeframes:sell,conflict_timeframes:conflicts});
}
